...
class OneDeck {
        ...
        for(int i = 1; i <= 4; i++) {
            for(int j = 1; j <= 13; j++) {
                try {
                    ...
                    //no need to handle exception since it will not happen
    ...
    {
        // implement this method!
        java.util.Collections.shuffle(playDeck);
    }
    ...
    {
        // implement this method!
        if (numberCards > remainSize()) throw new PlayingCardException("Not enough cards to deal");
        List<Card> dealtDeck = new ArrayList<Card>();
        for (int i = 0; i < numberCards; i++) {
            dealtDeck.add(playDeck.remove(0));
        }
        return dealtDeck;
    }
